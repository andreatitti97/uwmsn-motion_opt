...
def heuristicPenalty(choice, tmp_pi_bar, tmp_s, curren_cost, DT, ctrl_cmds, init_d, d_target):
    ...
    d_max, d_min = header.config.d*2, header.config.d/2
...
class Simple(pybnb.Problem):
    ...
    def save_state(self, node):
        ...
        node.state = (self._x_hat, self._s, self.value, self._bound, self.choices, self.pi_bar)

    def load_state(self, node):

        (self._x_hat, self._s, self.value, self._bound, self.choices, self.pi_bar) = node.state
    ...
    def branch(self): #durante il branch devi calcolare le varie realizzazioni quindi simuli qua
        ...
        for i in range(header.config.U):
            x, phi, tmp_s, tmp_pi_bar = self.simulation(self.ctrl_cmds[i], x_hat, self.P, s, self.sensors, self.v_n, self.DT, pi_bar, self.init_s_state)
            ...
            w_d = 1 #range cost functin always active
            w_g = header.utils.sig(d_target,self.init_d,self.alpha) #geometry cost function more relevant in the proximity of the target
            child_value = father_value + w_g*cost_g + w_d*d_target + penalty_d + penalty + penalty_abs

            # Branch the tree
            child = pybnb.Node()
            child.state = (x, tmp_s, child_value, self._bound, choices, tmp_pi_bar)
    ...
    def beliefPropagation(self, pi_bar,auvs_xy,ctrl_input,s_pose,v_n,DT):
        ...
        for i in range(self.auvNum):
            ...
            # Compute the path of the i-th AUV acoording to the choosen command
                    ...
                    ax_j = np.cos(j_pi_bar[2]+j_pi_bar[4])*j_pi_bar[3]*DT+j_pi_bar[0]
                    ay_j = np.sin(j_pi_bar[2]+j_pi_bar[4])*j_pi_bar[3]*DT+j_pi_bar[1]
                    auvs_xy[i] = [ax_j,ay_j,j_pi_bar[2]+j_pi_bar[4]]
                    out = auvs_xy[i]
...
'''if self.auvID == 1 and len(choices) == header.config.H:
                print('-------------------------------------------------')
                print('main cost function',cost_g)
                print('weighted main cost function',w_g*cost_g)
                print('cost function distance',d_target)
                print('penalty FDM',penalty)
                print('penalty distance neighbours',penalty_d)
